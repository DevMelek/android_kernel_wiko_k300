...
//#if defined(CONFIG_PROJECT_K400)
#include <mt_boot_common.h>
//#define CONFIG_RECEIVER_USE_PH_EX_PA
...
extern int AudDrv_GPIO_RCVSPK_Select(int bEnable);
/* VOW using */
	...
	if (enable) {
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == false
			...
			if (mIsExtSPKUse) {
				...
				Ana_Set_Reg(AUDTOP_CON4, 0x007C, 0xffff);//changed by kangqin from 0x005c to 0x007c for  L headphone used for speaker linein
			} else {
...
static void Voice_Amp_Change(bool enable)
{
	...
	if (enable) {
		#if defined(CONFIG_PROJECT_K400)
			#ifdef CONFIG_RECEIVER_USE_PH_EX_PA
...
#if defined(CONFIG_PROJECT_K300) || defined(CONFIG_PROJECT_K200)
...
//Xucs add start for PA mode select with perso start
#ifdef CONFIG_TINNO_PERSO_SENSORS
extern int g_PaMode;
#if defined(CONFIG_USE_EX_AMP_GPIO_DIRECT)
static void audio_mode_select(unsigned long pin, int mode)
{
    printk("g_Pamode is %d\n",g_PaMode);
    while(mode--) {
        mt_set_gpio_out(pin, GPIO_OUT_ZERO);	/* low disable */
        mdelay(GAP);
	 mt_set_gpio_out(pin, GPIO_OUT_ONE);	/* high enable */
	 mdelay(GAP);
    }
}
#else
static void audio_mode_select_dts(bool isDualSpk, int mode)
{
    printk("g_Pamode is %d\n",g_PaMode);
    while(mode--) {
	 if(isDualSpk){
...
#endif
#endif
//Xucs add start for PA mode select with perso end
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
...
#if defined(CONFIG_MTK_LEGACY)||defined(CONFIG_USE_EX_AMP_GPIO_DIRECT)
	int ret;
	...
	if (enable) {
		pr_debug("Ext_Speaker_Amp_Change ON+\n");
              ...
              if(g_PaMode == 0)
		    g_PaMode = 2;
	       printk("The value of  g_PaMode is %d\n",g_PaMode);
	       #endif
#if defined(CONFIG_PROJECT_K300) || defined(CONFIG_PROJECT_K200)|| !defined(CONFIG_MTK_SPEAKER)
#if defined(CONFIG_MTK_LEGACY)||defined(CONFIG_USE_EX_AMP_GPIO_DIRECT)
		...
		if (pin_extspkamp_2 != NULL_PIN_DEFINITION)
			mt_set_gpio_dir(pin_extspkamp_2, GPIO_DIR_OUT);	/* output */
	#ifdef CONFIG_PROJECT_K400
		 if(isHandFree && (FACTORY_BOOT != get_boot_mode())) {
		 	#ifndef CONFIG_TINNO_PERSO_SENSORS
		 	mt_set_gpio_out(pin_extspkamp, GPIO_OUT_ONE);
			udelay(GAP);
		 	mt_set_gpio_out(pin_extspkamp, GPIO_OUT_ZERO);
			udelay(GAP);
			...
			udelay(GAP);
			#else
			audio_mode_select(pin_extspkamp,g_PaMode);
			#endif
		 	}
		 else{
		 	#ifndef CONFIG_TINNO_PERSO_SENSORS
			...
			#else
			audio_mode_select(pin_extspkamp,g_PaMode);
			audio_mode_select(pin_extspkamp_2,g_PaMode);
			#endif
		 	}	 	
	#else
	...
	#ifdef CONFIG_PROJECT_K400
                if(isHandFree && (FACTORY_BOOT != get_boot_mode())) {
//if in handfree ,only open 1 speaker to avoid  sound echo
              #ifndef CONFIG_TINNO_PERSO_SENSORS
    		AudDrv_GPIO_EXTAMP_Select(true);
    		udelay(GAP);
    		AudDrv_GPIO_EXTAMP_Select(false);
    		...
    		AudDrv_GPIO_EXTAMP_Select(true);
		#else
		audio_mode_select_dts(false,g_PaMode);
		#endif
                } else {
              #ifndef CONFIG_TINNO_PERSO_SENSORS
    		AudDrv_GPIO_EXTAMP_Select(true);
		...
		AudDrv_GPIO_EXTAMP2_Select(true);
		#else
		audio_mode_select_dts(true,g_PaMode);
		#endif
                }
       #elif defined(CONFIG_PROJECT_K300)
		...
		audio_mode_select_dts(false,g_PaMode);
             ...
             pr_debug("ZXPA, before cmd 01 01 01 \n");
		#ifndef CONFIG_TINNO_PERSO_SENSORS
    		AudDrv_GPIO_EXTAMP_Select(false);
    		AudDrv_GPIO_EXTAMP2_Select(false);
    		udelay(GAP);
       ...
       #elif defined(CONFIG_PROJECT_K200)
        if(FACTORY_BOOT == get_boot_mode()) {
        //if in handfree ,only open 1 speaker to avoid  sound echo
             #ifndef CONFIG_TINNO_PERSO_SENSORS
    		AudDrv_GPIO_EXTAMP_Select(true);
		...
		#endif
	#endif
		pr_debug("AudDrv_GPIO_EXTAMP_Select(true)\n");
#endif /*CONFIG_MTK_LEGACY*/
...
#if defined(CONFIG_PROJECT_K300) || defined(CONFIG_PROJECT_K200) || !defined(CONFIG_MTK_SPEAKER)// 1 //ndef CONFIG_MTK_SPEAKER
		pr_warn("Ext_Speaker_Amp_Change ON set GPIO\n");
...
static void Headset_Speaker_Amp_Change(bool enable)
{
	if (enable) {
		...
		if (Speaker_mode == AUDIO_SPEAKER_MODE_D)
			...
			Speaker_ReveiverMode_Open();
#endif
        #if defined(CONFIG_PROJECT_K300) || defined(CONFIG_PROJECT_K200) // 1 //ndef CONFIG_MTK_SPEAKER
             //Speaker_Amp_Change(true);
		Ext_Speaker_Amp_Change(true);
        #endif // by zhangxian
