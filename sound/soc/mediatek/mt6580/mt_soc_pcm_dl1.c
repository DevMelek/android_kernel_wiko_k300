...
static unsigned int pin_extspkamp, pin_vowclk, pin_audclk, pin_audmiso,
	pin_audmosi, pin_i2s1clk, pin_i2s1dat, pin_i2s1mclk, pin_i2s1ws,pin_extspkamp2;
static unsigned int pin_mode_audclk, pin_mode_audmosi, pin_mode_audmiso,
	...
	pin_mode_i2s1mclk, pin_mode_i2s1ws,pin_mode_extspkamp2;
static unsigned int if_config1, if_config2, if_config3, if_config4, if_config5,
	if_config6, if_config7, if_config8, if_config9, if_config10;
#endif
#endif
...
static int Auddrv_OF_ParseGPIO(void *dev)
{
	...
	if (pdev->of_node) {
		...
		if (of_property_read_u32_index(pdev->of_node, "extspkamp2-gpio", 0, &pin_extspkamp2)) {
			if_config10 = 0;
			pr_err("extspkamp2-gpio get pin fail!!!\n");
		}
		if (of_property_read_u32_index(pdev->of_node, "extspkamp2-gpio", 1, &pin_mode_extspkamp2)) {
			if_config10 = 0;
			pr_err("extspkamp2-gpio get pin_mode fail!!!\n");
		}
...
int GetGPIO_Info(int type, int *pin, int *pinmode)
{
	...
	case 9:		/* pin_i2s1ws */
		...
		break;
	case 10:		/* pin_extspkamp */
		if (if_config10 == 1) {
			*pin = pin_extspkamp2 | 0x80000000;
			*pinmode = pin_mode_extspkamp2;
		} else {
			pr_err("GetGPIO_Info type %d fail!!!\n", type);
			*pin = -1;
			*pinmode = -1;
