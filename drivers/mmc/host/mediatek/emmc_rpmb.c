...
#if (defined(CONFIG_MICROTRUST_TEE_SUPPORT))
		//ret = mmc_blk_cmdq_switch_tmp(card, 0);
...
static long emmc_rpmb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if ((void *)arg == NULL) {
		MSG(ERR, "%s, user space arg is NULL!\n", __func__);
		return -1;
	}
#endif
	...
	if ((cmd == RPMB_IOCTL_SOTER_WRITE_DATA) || (cmd == RPMB_IOCTL_SOTER_READ_DATA)) {
		...
		ret = copy_from_user(&rpmb_size, (void *)arg, 4);
		if (ret < 0) {
			MSG(ERR, "%s, err=%x\n", __func__, err);
			return ret;
		}
		...
		if (rpmbinfor.size <= (RPMB_DATA_BUFF_SIZE - 4)) {
			...
			ret = copy_from_user(rpmb_buffer, (void *)arg, 4 + rpmbinfor.size);
			if (ret < 0) {
				MSG(ERR, "%s, err=%x\n", __func__, err);
				return ret;
			}
			...
			ret = -1;
			MSG(ERR, "%s, rpmbinfor.size(%d+4) is overflow (%d)!\n", __func__, rpmbinfor.size, (rpmbinfor.size + 4 - RPMB_DATA_BUFF_SIZE));
			return ret;
		}
	}
#endif
	...
	case RPMB_IOCTL_SOTER_WRITE_DATA:
		...
		ret = ut_rpmb_req_write_data(card, (struct s_rpmb *)(rpmbinfor.data_frame), rpmbinfor.size / RPMB_ONE_FRAME_SIZE);
	...
	case RPMB_IOCTL_SOTER_READ_DATA:
		...
		ret = ut_rpmb_req_read_data(card, (struct s_rpmb *)(rpmbinfor.data_frame), rpmbinfor.size / RPMB_ONE_FRAME_SIZE);
	...
	case RPMB_IOCTL_SOTER_GET_WR_SIZE:
		...
		if (ret) {
			MSG(ERR, "%s, Microtrust get max rpmb ioctl block size failed, error code (%x)\n", __func__, ret);
			return ret;
		}
