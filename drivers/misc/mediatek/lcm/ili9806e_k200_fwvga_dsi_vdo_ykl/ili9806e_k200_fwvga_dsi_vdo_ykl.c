...
#define LCM_DBG(fmt, arg...) \
	LCM_PRINT("[ili9806e_k200_fwvga_dsi_vdo_ykl.c] %s (line:%d) :" fmt "\r\n", __func__, __LINE__, ## arg)
 ...
 //edit by Magnum 2013-7-25 , solve esd read id error
  ...
  // ui = (1000 * div2 * div1 * pre_div * post_div)/ (fbk_sel * (fbk_div+0x01) * 26 * 2) + 1;
		...
		params->dsi.ssc_range=2;
	       params->dsi.PLL_CLOCK				= 211;//170;//222;		
...
static unsigned int lcm_compare_id(void)
{
    ...
    //if ((LCM_ID == id)&&(lcm_vol >= MIN_VOLTAGE)&&(lcm_vol <= MAX_VOLTAGE)){
...
LCM_DRIVER ili9806e_k200_fwvga_dsi_vdo_ykl_lcm_drv = 
{
    .name			= "ili9806e_fwvga_dsi_vdo_ykl",
