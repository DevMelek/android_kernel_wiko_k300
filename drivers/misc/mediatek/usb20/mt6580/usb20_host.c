...
// TINNO BEGIN
#ifdef CONFIG_TINNO_PERSO_SENSORS // wenguangyu, add for perso hardware configD
    printk(KERN_INFO "Tinnod Board Config g_iOTG_open = %d\n",g_iOTG_open);
    if (mtk_musb->is_host && g_iOTG_open) {
#else
    if (mtk_musb->is_host) {
#endif
// TINNO END
...
static void otg_int_init(void)
{
	...
	if (IS_ERR(pinctrl_iddig)) {
		...
		dev_err(mtk_musb->controller, "zhangxian: Cannot find usb pinctrl iddig_irq_init\n");
	}
