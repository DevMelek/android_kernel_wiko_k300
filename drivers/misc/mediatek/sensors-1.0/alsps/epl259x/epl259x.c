...
#define LUX_PER_COUNT	250  //ALS lux per count
...
{
    struct alsps_hw  hw; // Jiangde, *hw-->hw
...
int epl_sensor_setup_eint(struct i2c_client *client)
{
	...
	pr_err("HJDDbgAlsPs, use client->dev");
...
#if 0
int get_alsps_dts_func_epl259x(struct device_node *node,  struct alsps_hw *hw)
{
	int i, ret;
	u32 i2c_num[] = { 0 };
...
/*----------------------------------------------------------------------------*/
int val_for_tp_ps_en = 0;
extern  int tp_ps_open_report_data(int open);
extern int tp_ps_enable_nodata(int en);
extern int tp_ps_set_delay(u64 ns);
extern int tp_ps_get_data(int *value, int *status);
extern int tp_ps_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs);
extern int tp_ps_flush(void);
...
static int epl_sensor_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if 1
	struct ps_control_path tp_ps_ctl={0};
	struct ps_data_path tp_ps_data={0};
#endif
    ...
    pr_err("HJDDbgAlsPs, at %s--%d!, renvon=0x%x \n", __func__, __LINE__, renvo);
	

	val_for_tp_ps_en = 0;
    if(1 != epl_sensor_I2C_Read(client, 0x21, 2))epl_sensor_I2C_Read(client, 0x21, 2);
	...
	{
	       val_for_tp_ps_en = 1;  // no IC
		err = -ENOMEM;
		//goto exit_init_failed;
           goto exit;
	}
     printk("step 1 :val_for_tp_ps = %d\n", val_for_tp_ps_en);
 ...
 if (1 == val_for_tp_ps_en){

	tp_ps_ctl.is_use_common_factory = false;
	tp_ps_ctl.open_report_data = tp_ps_open_report_data;
	tp_ps_ctl.enable_nodata = tp_ps_enable_nodata;
	tp_ps_ctl.set_delay = tp_ps_set_delay;
	tp_ps_ctl.batch = tp_ps_batch;
	tp_ps_ctl.flush = tp_ps_flush;
	tp_ps_ctl.is_report_input_direct = false;
#ifdef CUSTOM_KERNEL_SENSORHUB
	tp_ps_ctl.is_support_batch = obj->hw.is_batch_supported_ps;
#else
	tp_ps_ctl.is_support_batch = false;
#endif

	err = ps_register_control_path(&tp_ps_ctl);
	if (err) {
		APS_ERR("register fail = %d\n", err);
	}

	tp_ps_data.get_data = tp_ps_get_data;
	tp_ps_data.vender_div = 100;
	err = ps_register_data_path(&tp_ps_data);
	if (err) {
...
#if ANDR_M
#else /*ANDR_M*/
    // i2c_register_board_info(hw.i2c_num, &i2c_epl_sensor, 1); // Jiangde-- NULL pointer?
#endif /*ANDR_M*/
