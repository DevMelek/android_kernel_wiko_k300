...
static int ltr559_als_enable(int gainrange)
{
	...
	ltr559_i2c_read_reg(LTR559_ALS_CONTR);
...
static int ltr559_init_device(void)
{
	...
	error = ltr559_i2c_write_reg(LTR559_PS_LED, 0x1F/*0x7F*/);
	if (error < 0) {
		APS_LOG("ltr559_ps_enable() error3...\n");
		return error;
	}

	error = ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 0x0F);	/* modify by hzb */
	if (error < 0) {
...
static void ps_cali_set_threshold(void)
{
    ...
    ltr559_ps_factory_set_threashold(value_thres);
}
...
static int ltr559_ps_factory_enable_calibration(void)
{
    ...
    APS_ERR("  entry ltr559_ps_factory_enable_calibration \n");
...
exit_sensor_obj_attach_fail:
    //misc_deregister(&ltr559_device); //LINE<JIRA_ID><DATE20180109><add ftm alsps and ps cali>zenghaihui
