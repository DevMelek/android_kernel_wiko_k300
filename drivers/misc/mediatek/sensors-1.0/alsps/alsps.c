...
static int alsps_real_driver_init(void)
{
    ...
    SORT_LOAD_BY_NAME(alsps_init_list, SIMULATOR_PS_DEVICE, MAX_CHOOSE_ALSPS_NUM);
    #endif
	...
	for (i = 0; i < MAX_CHOOSE_ALSPS_NUM; i++) {
		...
		if (alsps_init_list[i] != 0) {
            ...
            if (err == 0) {
                ...
                #ifdef CONFIG_TINNO_PRODUCT_INFO
                FULL_PRODUCT_DEVICE_INFO(ID_PSENSOR, alsps_init_list[i]->name);
                #endif
				break;
			}
		}
...
static int __init alsps_init(void)
{
    ...
    #if !defined(CONFIG_TINNO_CTP)
        if (0 == g_iLight_sensor && 0 == g_iProximity_sensor)
        {
            printk(KERN_INFO "[alsps_dev]: 0 == g_iLight_sensor && 0 == g_iProximity_sensor! \n");
            return 0;
        }
    #endif // CONFIG_PROJECT_K200
#endif
