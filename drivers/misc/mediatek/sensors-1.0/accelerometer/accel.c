	...
	for (i = 0; i < MAX_CHOOSE_G_NUM; i++) {
		...
		if (gsensor_init_list[i] != 0) {
			...
			if (err == 0) {
                ...
                #ifdef CONFIG_TINNO_PRODUCT_INFO
                FULL_PRODUCT_DEVICE_INFO(ID_GSENSOR, gsensor_init_list[i]->name);
                #endif
				break;
			}
		}
...
static int __init acc_init(void)
{
    ...
    printk(KERN_INFO "Tinnod Board Config, g_iAcceleration_sensor = %d\n", g_iAcceleration_sensor);
    if (g_iAcceleration_sensor == 0)
    {
        printk(KERN_INFO "[acc_dev]: g_iAcceleration_sensor == 0! \n");
        return 0;
    }
#endif
