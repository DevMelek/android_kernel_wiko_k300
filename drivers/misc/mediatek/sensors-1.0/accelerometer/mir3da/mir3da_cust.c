...
static int mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_err("mir3da_probe line=%d. \n", __LINE__);
	result = accel_factory_device_register(&mir3da_factory_device); // Jiangde, Pendings
	if (result) {
		pr_err("acc_factory register failed result=%d.\n", result);
#ifndef CONFIG_PROJECT_K100
        goto exit_misc_device_register_failed; // Jiangde, Pendings
#endif
	}
