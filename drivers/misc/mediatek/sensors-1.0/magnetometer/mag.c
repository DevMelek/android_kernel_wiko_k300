	...
	for (i = 0; i < MAX_CHOOSE_G_NUM; i++) {
		...
		if (msensor_init_list[i] != 0) {
			...
			if (err == 0) {
                ...
                #ifdef CONFIG_TINNO_PRODUCT_INFO
                FULL_PRODUCT_DEVICE_INFO(ID_MSENSOR, msensor_init_list[i]->name);
                #endif
				break;
			}
		}
...
static int __init mag_init(void)
{
    ...
    printk(KERN_INFO "Tinnod Board Config, g_iMagnetic_sensor = %d\n", g_iMagnetic_sensor);
    if (g_iMagnetic_sensor == 0)
    {
        printk(KERN_INFO "[mag_dev]: g_iMagnetic_sensor == 0! \n");
        return 0;
    }
#endif
