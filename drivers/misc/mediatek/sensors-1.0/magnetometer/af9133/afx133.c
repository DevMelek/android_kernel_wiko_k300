...
static int afx133_ReadSensorData(int *buf)
{
  ...
  // pr_err("HJDDbgMag, 1 output[0]=%d, output[1]=%d, output[2]=%d \n", output[0], output[1], output[2]);
  
  for(i=0;i<3;i++) output[i] = (output[i] > 32767) ? (output[i] - 65536) : output[i];
  
  // pr_err("HJDDbgMag, 2 output[0]=%d, output[1]=%d, output[2]=%d \n", output[0], output[1], output[2]);
  ...
  // pr_err("HJDDbgMag, xyz[0]=%d, xyz[1]=%d, xyz[2]=%d \n", xyz[0], xyz[1], xyz[2]);
  ...
  // pr_err("HJDDbgMag22, afx133_comp[0]=%d, afx133_comp[1]=%d, afx133_comp[2]=%d \n", afx133_comp[0], afx133_comp[1], afx133_comp[2]);
  // pr_err("HJDDbgMag2, buf[0]=%d, buf[1]=%d, buf[2]=%d \n", buf[0], buf[1], buf[2]);
 ...
 /*----------------------------------------------------------------------------*/
static int afx133_AxisInfoToPat(
...
/*----------------------------------------------------------------------------*/
static int16_t afx133_SetCert(void)
{
    struct i2c_client *client = afx133_i2c_client;
    struct afx133_i2c_data *data = i2c_get_clientdata(client);
    ...
    axis_sign[2] = 0;
#endif
    ret = afx133_AxisInfoToPat(axis_order, axis_sign, &cert);
    if (ret != 0)
        return 0;
    return cert;
}
...
static int afx133_get_data(int *x,int *y, int *z,int *status)
{
  struct afx133_i2c_data *data ;
    ...
    // pr_err("HJDDbgMag, afx133_get_data in \n");
    if(NULL == afx133_i2c_client)
    {
        MSE_ERR("afx133_i2c_client IS NULL !\n");
        return -1;
    ...
    if(err)
    {
        pr_err("afx133_get_data: ReadSensorData fail\n");
        return err;
    }
    
    // afx133_coordinate_convert(xyz); Do NOT convert!
    
    *x = xyz[0] * CONVERT_M;
    *y = xyz[1] * CONVERT_M;
    *z = xyz[2] * CONVERT_M;
    *status = 1;
    // pr_err("HJDDbgMag22, x=%d, y=%d, z=%d \n", *x, *y, *z);
...
static int afx133_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  err = get_mag_dts_func(client->dev.of_node, &data->hw);
    ...
    //ctl_path.libinfo.layout = afx133_SetCert();
    pr_err("HJDDbgMag, layout=%d", data->hw.direction);
