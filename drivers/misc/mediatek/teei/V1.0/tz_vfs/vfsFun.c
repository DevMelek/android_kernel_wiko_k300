			...
			unsigned int cmd, unsigned long arg)
{
	...
	case TEEI_VFS_NOTIFY_DRM:
		IMSG_DEBUG("***************TEEI_VFS_NOTIFY_DRM\n");
		if (copy_from_user((void *)&drm_driver_id, (void *)arg, sizeof(unsigned int)))
			return -EFAULT;
		IMSG_DEBUG("get notification from secure driver 0x%x\n", drm_driver_id);

		/* notify drm driver */
		ret = tz_sec_drv_notification(drm_driver_id);
		IMSG_DEBUG("get notification done 0x%x, ret %d\n", drm_driver_id, ret);
	...
	case GET_FP_VENDOR_NAME_CMD: {
		...
		IMSG_INFO("%s:%d get_fp_ta_load_path ok , fp_ta_load_path = %s",__func__, __LINE__, fp_ta_load_path);
