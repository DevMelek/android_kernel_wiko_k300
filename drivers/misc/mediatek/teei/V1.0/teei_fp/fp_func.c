...
int fp_open(struct inode *inode, struct file *filp)
{
	if (wait_teei_config_flag == 1) {
		...
		ret = wait_event_timeout(__fp_open_wq, (teei_config_flag == 1), msecs_to_jiffies(1000 * 50));
		if (ret == 0) {
...
#ifdef FP_DEBUG
	IMSG_ERROR("##################################\n");
	IMSG_ERROR("fp ioctl received received cmd is: %x arg is %x\n", cmd, (unsigned int)arg);
	IMSG_ERROR("CMD_MEM_CLEAR is: %x CMD_FP_CMD is %x\n", CMD_MEM_CLEAR, CMD_FP_CMD);
#endif
	switch (cmd) {
...
#ifdef FP_DEBUG
		IMSG_ERROR("invoke fp cmd CMD_FP_CMD: arg's address is %x, args's length %d\n",
			(unsigned int)arg, args_len);
		IMSG_ERROR("invoke fp cmd fp_cid is %d fp_fid is %d\n", fp_cid, fp_fid);
#endif
		if (!fp_buff_addr) {
...
#ifdef FP_DEBUG
		IMSG_ERROR("back from TEEI try copy share mem to user\n");
		IMSG_ERROR("result in share memory %d\n", *((unsigned int *)fp_buff_addr));
		IMSG_ERROR("[%s][%d] fp_buff_addr 88 - 91 = %d\n", __func__, args_len,
			*((unsigned int *)(fp_buff_addr + 88)));
#endif
...
#ifdef FP_DEBUG
		IMSG_ERROR("result after copy %d\n", *((unsigned int *)arg));
		IMSG_ERROR("invoke fp cmd end.\n");
#endif
		break;
	case CMD_FP_GATEKEEPER_CMD:
		#ifdef FP_DEBUG
		IMSG_ERROR("case CMD_FP_GATEKEEPER_CMD\n");
		#endif
...
#ifdef FP_DEBUG
		IMSG_ERROR("varify gatekeeper_buff_addr ok\n");
		IMSG_ERROR("the value of gatekeeper_buff_addr is %lu\n", gatekeeper_buff_addr);
#endif
