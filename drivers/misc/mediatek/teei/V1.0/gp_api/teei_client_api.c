...
inline unsigned int params_null_check(void *paras, char *error_log)
{
	if (paras == NULL) {
		IMSG_ERROR("[E] Error Illegal argument : %s\n", error_log);
		return -1;
	}
	return 0;
}

inline unsigned int release_share_memory(struct UT_TEEC_SharedMemory *memory, int *ret)
{
...
*
* @param context: A struct TEEC_Context structure that MUST be initialized by the
* Implementation.
*
* @return unsigned int:
...
{
	struct UT_TEEC_Context *ut_context = NULL;
	struct ctx_data ctx;
	long dev_file_id = 0;
	char temp_name[256];
	int retVal = 0;
	...
	if (dev_file_id < 0) {
		...
		ut_context = kmalloc(sizeof(struct UT_TEEC_Context), GFP_KERNEL);
		if (ut_context == NULL)
			return TEEC_ERROR_OUT_OF_MEMORY;

		memset(ut_context, 0x0, sizeof(struct UT_TEEC_Context));
 ...
 *
 * @param context: An initialized struct TEEC_Context structure which is to be
 * finalized.
 */
void TEEC_FinalizeContext(struct TEEC_Context *context)
{
	...
	struct ctx_data ctx;
	struct UT_TEEC_SharedMemory *memory_item = NULL;
	struct UT_TEEC_Session *session_item = NULL;
	...
	list_for_each_entry(memory_item, &(context->imp->shared_mem_list), head_ref) {
		...
		retVal = ut_client_shared_mem_free(memory_item->context->fd,
					(struct teei_session_shared_mem_info *)(&info));
		if (retVal != 0)
			result = 1;
	}
	if (result == 0)
	...
	if (list_empty(&(context->imp->session_list)) && list_empty(&(context->imp->shared_mem_list))) {
		...
		if (retVal != 0) {
			IMSG_ERROR("[%s]:ioctl failed, return value is %d\n", __func__, retVal);
			return;
		}
...
{
	struct UT_TEEC_SharedMemory *uTSharedMem = NULL;
	...
	uTSharedMem = kmalloc(sizeof(struct UT_TEEC_SharedMemory), GFP_KERNEL);
	if (uTSharedMem == NULL)
		return TEEC_ERROR_OUT_OF_MEMORY;
...
inline unsigned int set_inout(unsigned int type, unsigned int flag)
{
...
inline unsigned int ut_enc_value(struct client_encode_cmd *enc, int dev_file_id,
								unsigned int *returnOrigin, unsigned int inout)
{
	unsigned int retVal = 0;

	enc->len  = sizeof(unsigned int);
...
unsigned int ut_dec_value(struct client_encode_cmd *enc, int dev_file_id, unsigned int *returnOrigin)
{
	unsigned int retVal = 0;
...
unsigned int ut_enc_register_data(int dev_file_id, struct client_encode_cmd *enc,
								union TEEC_Parameter params, unsigned int *returnOrigin,
								uint8_t isTemp)
{
	unsigned int retVal = 0;
...
inline unsigned int ut_enc_memory(int dev_file_id, struct client_encode_cmd *enc,
								union TEEC_Parameter params, unsigned int *returnOrigin,
								unsigned int inout, uint8_t isTemp)
{
	unsigned int retVal = 0;
	...
	unsigned int *returnOrigin)
{
	...
	struct client_encode_cmd enc;
	struct UT_TEEC_Session *imp = NULL;
	...
	/* init ut session */
	imp = kmalloc(sizeof(struct UT_TEEC_Session), GFP_KERNEL);
	memset(imp, 0x0, sizeof(struct UT_TEEC_Session));
	session->imp = imp;
		...
		for (param_count = 0; param_count < 4; param_count++) {
			...
			/* check temp memory type */
				...
				if (operation->params[param_count].tmpref.size == 0) {
					...
					retVal = TEEC_ERROR_NO_DATA;
					IMSG_ERROR("TEEC_OpenSession:temporary memory  is not supported\n");
					break;
				}
				enc.len = operation->params[param_count].tmpref.size;
				if (enc.len == 0) {
					temp_enc_data = (unsigned long)ut_client_map_mem(
									session->imp->device->fd, ZERO_SIZE_MAP);
					enc.data = temp_enc_data;
				} else {
					temp_enc_data = (unsigned long)ut_client_map_mem(
									session->imp->device->fd, enc.len);
					enc.data = temp_enc_data;
				}
				...
				memcpy((void *)(enc.data), operation->params[param_count].tmpref.buffer,
						operation->params[param_count].tmpref.size);
				retVal = ut_enc_memory(context->imp->fd, &enc,
										operation->params[param_count],
										returnOrigin, inout, 1);
				if (retVal != 0) {
					session->imp->s_errno = retVal;
					break;
				}
		...
		for (param_count = 0; param_count < 4; param_count++) {
			if ((param_types[param_count] == TEEC_VALUE_INOUT) ||
				...
				if (retVal != 0) {
					...
					operation->params[param_count].value.a = *((unsigned int *)enc.data);
				...
				if (retVal != 0) {
					...
					operation->params[param_count].value.b = *((unsigned int *)enc.data);
				...
				retVal = ut_client_decode_array_space(context->imp->fd,
				...
				if (retVal != 0) {
					if (returnOrigin != NULL)
						*returnOrigin = TEEC_ORIGIN_COMMS;
					session->imp->s_errno = -(retVal);
					IMSG_ERROR("TEEC_InvokeCommand  failed\n");
					break;
				}
...
operation_release:
	...
	if (operation != NULL) {
		retVal = ut_client_operation_release(context->imp->fd,
						(struct teei_client_encode_cmd *)&enc);
		if (retVal != 0) {
			if (returnOrigin != NULL)
				*returnOrigin = TEEC_ORIGIN_COMMS;
								...
								unsigned int *returnOrigin)
{
	...
	unsigned int param_types[4], param_count;
	struct client_encode_cmd enc;
	void *temp_buffer[4];
	unsigned long temp_enc_data = 0;
			...
			/* start if for TEEC_MEM_REF check */
				...
				if (param_types[param_count] == TEEC_MEMREF_PARTIAL_INPUT) {
					if (!(operation->params[param_count].memref.parent->flags & TEEC_MEM_INPUT)) {
						...
						retVal = TEEC_ERROR_BAD_FORMAT;
						IMSG_ERROR("TEEC_InvokeCommand: memory is invalid\n");
						break;
					}
				}
				...
				if ((param_types[param_count] == TEEC_MEMREF_PARTIAL_INPUT) ||
					 ...
					 *	IMSG_ERROR("TEEC_InvokeCommand: memory reference offset +
				...
				enc.len  = operation->params[param_count].tmpref.size;
				if (enc.len == 0)
					temp_buffer[param_count] =
					ut_client_map_mem(session->imp->device->fd, ZERO_SIZE_MAP);
				else
	...
	if ((operation != NULL) && (operation->paramTypes != 0)) {
		for (param_count = 0; param_count < 4; param_count++) {
			...
			if ((param_types[param_count] == TEEC_VALUE_INOUT) ||
				...
				if (inout == 0)
					continue;
				retVal = ut_client_decode_array_space(session->imp->device->fd,
								 (struct teei_client_encode_cmd *)(&enc));
