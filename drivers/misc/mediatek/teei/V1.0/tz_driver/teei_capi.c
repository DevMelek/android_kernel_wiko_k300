		...
		if (temp_cont->cont_id == dev_file_id) {
			list_for_each_entry(temp_ses, &temp_cont->sess_link, link) {
				if (temp_ses->sess_id == ses_close->session_id) {
					retVal = teei_client_close_session_for_service((void *)dev_file_id, temp_ses);
					goto out;
				}
			}
		}
				...
				struct teei_session **psession)
{
	...
	if (!enc_found) {
		...
		enc_context->meta = tz_malloc_shared_mem(sizeof(struct teei_encode_meta) *
						(TEEI_MAX_RES_PARAMS + TEEI_MAX_REQ_PARAMS), GFP_KERNEL);
...
int __teei_client_send_cmd(unsigned long dev_file_id, struct teei_client_encode_cmd *enc)
{
	...
	if (enc->encode_id != -1) {
		...
		retVal = teei_client_prepare_encode((void *)dev_file_id, enc, &enc_temp, &temp_ses);
		if (retVal == 0)
			enc_found = 1;
	}
...
int __teei_client_encode_uint32(unsigned long dev_file_id, struct teei_client_encode_cmd *enc, int kernel_range)
{
	...
	retVal = teei_client_prepare_encode((void *)dev_file_id, enc, &enc_context, &session);
	if (retVal != 0) {
	...
	if (enc->param_type == TEEIC_PARAM_IN) {
		...
		if ((enc_context->enc_req_offset + sizeof(u32) <= TEEI_1K_SIZE) &&
			...
			if (kernel_range == 0) {
				...
				unsigned long result = copy_from_user(&value, pt, 4);
				if (result != 0) {
		...
		if ((enc_context->enc_req_offset + sizeof(u32) <= TEEI_1K_SIZE) &&
		   (enc_context->enc_req_pos < TEEI_MAX_REQ_PARAMS)) {
			...
			if (kernel_range == 0) {
				...
				unsigned long  result = copy_from_user(&value, pt, 4);
				if (result) {
					IMSG_ERROR("[%s][%d] failed errno = %ld!\n", __func__, __LINE__, result);
				}
			} else
				memcpy(&value, (const void *)addr, 4);
			*/
...
int __teei_client_encode_array(unsigned long dev_file_id, struct teei_client_encode_cmd *enc, int kernel_range)
{
	...
	if (enc->param_type == TEEIC_PARAM_IN) {
		if (NULL == enc_context->ker_req_data_addr) {
			/* IMSG_ERROR("[%s][%d] allocate req data data.\n", __func__, __LINE__); */
			enc_context->ker_req_data_addr = tz_malloc_shared_mem(TEEI_1K_SIZE, GFP_KERNEL);
			if (!enc_context->ker_req_data_addr) {
				IMSG_ERROR("[%s][%d] enc_context->ker_req_data_addr is NULL!\n", __func__, __LINE__);
				return -ENOMEM;
		...
		if ((enc_context->enc_req_offset + enc->len <= TEEI_1K_SIZE) &&
			...
			if (kernel_range == 0) {
				if (copy_from_user((char *)enc_context->ker_req_data_addr + enc_context->enc_req_offset,
						(const void __user *)enc->data, enc->len)) {
					IMSG_ERROR("[%s][%d] copy from user failed.\n", __func__, __LINE__);
					return -EFAULT;
				}
			} else {
...
int __teei_client_encode_mem_ref(unsigned long dev_file_id, struct teei_client_encode_cmd *enc, int kernel_range)
{
	...
	list_for_each_entry(temp_shared_mem, &session->shared_mem_list, s_head) {
		u64 addr = enc->data;
		if (temp_shared_mem && ((unsigned long)temp_shared_mem->index == (unsigned long)addr)) {
			IMSG_ERROR("[%s][%d] found the shared mem\n", __func__, __LINE__);
			shared_mem_found = 1;
			break;
		}
	}
...
int __teei_client_encode_mem_ref_64bit(unsigned long dev_file_id, struct teei_client_encode_cmd *enc, int kernel_range)
{
	...
	list_for_each_entry(temp_shared_mem, &session->shared_mem_list, s_head) {
		/*u64 addr = enc->data;*/
		if (temp_shared_mem && ((unsigned long long)temp_shared_mem->index == enc->data)) {
			shared_mem_found = 1;
			break;
		}
	}
		...
		list_for_each_entry(temp_cont,
			...
			if (temp_cont->cont_id == (unsigned long)dev_file_id) {
				list_for_each_entry(temp_shared_mem,
						&temp_cont->shared_mem_list,
						head) {
					if ((unsigned long long)(temp_shared_mem->index) == enc->data) {
						shared_mem_found = 1;
						break;
					}
				}
...
int __teei_client_decode_uint32(unsigned long dev_file_id, struct teei_client_encode_cmd *dec, int kernel_range)
{
	...
	retVal = teei_client_prepare_decode((void *)dev_file_id, dec, &dec_context);
	...
	if ((dec_context->dec_res_pos <= dec_context->enc_res_pos) &&
	   (dec_context->meta[dec_context->dec_res_pos].type == TEEI_ENC_UINT32)) {
		if (dec_context->meta[dec_context->dec_res_pos].usr_addr) {
			dec->data = ((uint64_t)(dec_context->meta[dec_context->dec_res_pos].usr_addr));
		}

		/*void __user *pt = NULL;*/

		if (((u32 *)dec->data) == NULL) {
			IMSG_ERROR("[%s][%d] error decode dec.data addr11111 is NULL!\n", __func__, __LINE__);
		}

		if (((u32 *)((char *)dec_context->ker_res_data_addr + dec_context->dec_offset) == NULL)) {
			IMSG_ERROR("[%s][%d] decode data decode addr11111 is NULL!\n", __func__, __LINE__);
		} else {
	...
	if ((dec_context->dec_res_pos <= dec_context->enc_res_pos) &&
		...
		if (dec_context->meta[dec_context->dec_res_pos].len >=
			...
			if (kernel_range == 0) {
				...
				memcpy((void *)dec->data, (char *)dec_context->ker_res_data_addr + dec_context->dec_offset,
						dec_context->meta[dec_context->dec_res_pos].ret_len);
			}
		} else {
		...
		if (dec_context->meta[dec_context->dec_res_pos].len >=
				dec_context->meta[dec_context->dec_res_pos].ret_len) {
			dec->data = dec_context->meta[dec_context->dec_res_pos].usr_addr;
			pmem = (unsigned long)(dec_context->ker_res_data_addr) + dec_context->dec_offset;
			...
			IMSG_ERROR("[%s][%d] buffer length is small. Length required %x and supplied length %x",
					__func__, __LINE__,
					dec_context->meta[dec_context->dec_res_pos].ret_len,
					dec_context->meta[dec_context->dec_res_pos].len);
	...
	list_for_each_entry(temp_cont,
		...
		if (temp_cont->cont_id == dev_file_id) {
			list_for_each_entry_safe(temp_shared_mem,
					...
					&temp_cont->shared_mem_list,
					head) {
				if (temp_shared_mem && (unsigned long)temp_shared_mem->u_addr == (mem_info->user_mem_addr)) {
...
void *__teei_client_map_mem(unsigned long dev_file_id, unsigned long size, unsigned long user_addr)
{
	...
	/*IMSG_DEBUG("[%s][%d] dev_file_id is %d\n", __func__, __LINE__, dev_file_id);*/
