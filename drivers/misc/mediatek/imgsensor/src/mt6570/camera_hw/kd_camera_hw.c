...
#define VOL_1000 1000000
	...
	if (On) {
		...
		//begin add for k100
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP250A_CMK_K100_MIPI_RAW, currSensorName))) {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC030A_CMK_K100_MIPI_RAW, currSensorName))) {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_ON]);
			mdelay(50);
