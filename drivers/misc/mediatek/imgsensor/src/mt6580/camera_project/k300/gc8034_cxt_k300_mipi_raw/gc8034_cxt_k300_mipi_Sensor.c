...
#include "gc8034_cxt_k300_mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC8034_CXT_K300__camera_sensor"
#define LOG_1 LOG_INF("GC8034_CXT_K300_,MIPI 4LANE\n")
/****************************   Modify end    *******************************************/
 
#define GC8034_CXT_K300_DEFAULT_DUMMY_PIXEL_NUMS   0x216 // HB
#define GC8034_CXT_K300_DEFAULT_DUMMY_LINE_NUMS    0x10 //VB
static kal_uint32 Basic_Dummy_Line = GC8034_CXT_K300_DEFAULT_DUMMY_LINE_NUMS;

#define LOG_INF(format, args...)    pr_err(PFX "[%s] " format, __FUNCTION__, ##args)



kal_bool GC8034_CXT_K300_DuringTestPattern = KAL_FALSE;
...
//#define GC8034_CXT_K300_OTP_DEBUG

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC8034_CXT_K300_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
typedef struct otp_gc8034_cxt_k300
{
	kal_uint16  dd_cnt;
	kal_uint16  dd_flag;
#ifdef GC8034_CXT_K300_OTP_FOR_CUSTOMER
...
static gc8034_cxt_k300_otp gc8034_cxt_k300_otp_info;
#ifdef GC8034_CXT_K300_OTP_FOR_CUSTOMER
u16 otp_gc8034_cxt_k300_af_data[2];
static kal_uint8 LSC_ADDR[4]={0x0e,0x20,0x1a,0x88};
#endif

static kal_uint8 gc8034_cxt_k300_read_otp(kal_uint8 page, kal_uint8 addr)
{
	LOG_INF("E!\n");
...
static void gc8034_cxt_k300_read_otp_group(kal_uint8 page, kal_uint8 addr, kal_uint8 *buff, int size)
{
	kal_uint8 i;
...
static void gc8034_cxt_k300_gcore_read_otp_info(void)
{
	kal_uint8 flagdd = 0;
	kal_uint16 i = 0,j = 0;
	kal_uint8 temp = 0;
	...
	kal_uint8 af[4]={0};
#endif
#ifdef GC8034_CXT_K300_OTP_DEBUG
	kal_uint8  debug[128] = { 0 };
#endif 
	memset(&gc8034_cxt_k300_otp_info, 0, sizeof(gc8034_cxt_k300_otp_info));
	/*TODO*/
	/* Static Defective Pixel */
	flagdd = gc8034_cxt_k300_read_otp(0, 0x0b);
	LOG_INF("GC8034_CXT_K300__OTP_DD : flag_dd = 0x%x\n",flagdd);

	//DD
	switch(flagdd&0x03)   
	{
	case 0x00:
		LOG_INF("GC8034_CXT_K300__OTP_DD is Empty !!\n");
		gc8034_cxt_k300_otp_info.dd_flag = 0x00;
		break;
	case 0x01:
		LOG_INF("GC8034_OTP_DD is Valid!!\n");
		gc8034_cxt_k300_otp_info.dd_cnt = gc8034_cxt_k300_read_otp(0, 0x0c) + gc8034_cxt_k300_read_otp(0, 0x0d);
		gc8034_cxt_k300_otp_info.dd_flag = 0x01;
		LOG_INF("GC8034_OTP : total_number = %d\n", gc8034_cxt_k300_otp_info.dd_cnt);
		break;
	case 0x02:
	case 0x03:	
		LOG_INF("GC8034_CXT_K300__OTP_DD is Invalid !!\n");
		gc8034_cxt_k300_otp_info.dd_flag = 0x02;
		break;
	default :
		break;
	}
...
#ifdef GC8034_CXT_K300_OTP_FOR_CUSTOMER	
	flag_module = gc8034_cxt_k300_read_otp(9, 0x6f);
	flag_wb = gc8034_cxt_k300_read_otp(9, 0x5e);
	flag_af = gc8034_cxt_k300_read_otp(3, 0x3a);
	LOG_INF("GC8034_CXT_K300__OTP : flag_module = 0x%x , flag_wb = 0x%x ,flag_af = 0x%x\n",flag_module,flag_wb,flag_af);
...
//INFO&WB&AF
	for(index=0;index<2;index++)
	{
		...
		{
		case 0x00:
			LOG_INF("GC8034_CXT_K300__OTP_INFO group %d is Empty !!\n", index + 1);
			break;
		case 0x04:
			LOG_INF("GC8034_CXT_K300__OTP_INFO group %d is Valid !!\n", index + 1);
			check = 0;
			gc8034_cxt_k300_read_otp_group(9, INFO_ROM_START + index * INFO_WIDTH, &info[0], INFO_WIDTH);
			...
			{
				gc8034_cxt_k300_otp_info.module_id = info[0];
				gc8034_cxt_k300_otp_info.lens_id = info[1];
				gc8034_cxt_k300_otp_info.vcm_driver_id = info[2];
				gc8034_cxt_k300_otp_info.vcm_id = info[3];
				gc8034_cxt_k300_otp_info.year = info[4];
				gc8034_cxt_k300_otp_info.month = info[5];
				gc8034_cxt_k300_otp_info.day = info[6];
			}
			else
			{
				LOG_INF("GC8034_CXT_K300__OTP_INFO Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x08:
		case 0x0c:	
			LOG_INF("GC8034_CXT_K300__OTP_INFO group %d is Invalid !!\n", index + 1);
			break;
		default :
			break;
		}
		...
		{
		case 0x00:
			LOG_INF("GC8034_CXT_K300__OTP_WB group %d is Empty !!\n", index + 1);
			gc8034_cxt_k300_otp_info.wb_flag = gc8034_cxt_k300_otp_info.wb_flag|0x00;
			break;
		case 0x04:
			LOG_INF("GC8034_CXT_K300__OTP_WB group %d is Valid !!\n", index + 1);	
			check = 0;
			gc8034_cxt_k300_read_otp_group(9, WB_ROM_START + index * WB_WIDTH, &wb[0], WB_WIDTH);
			...
			{
				gc8034_cxt_k300_otp_info.rg_gain = (wb[0] | ((wb[1] & 0xf0) << 4)) > 0 ?
					(wb[0] | ((wb[1] & 0xf0) << 4)) : 0x400;
				gc8034_cxt_k300_otp_info.bg_gain = (((wb[1] & 0x0f) << 8) | wb[2]) > 0 ?
					(((wb[1] & 0x0f) << 8) | wb[2]) : 0x400;
				gc8034_cxt_k300_otp_info.wb_flag = gc8034_cxt_k300_otp_info.wb_flag|0x01;
			}
			else
			{
				LOG_INF("GC8034_CXT_K300__OTP_WB Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x08:
		case 0x0c:	
			LOG_INF("GC8034_CXT_K300__OTP_WB group %d is Invalid !!\n", index + 1);			
			gc8034_cxt_k300_otp_info.wb_flag = gc8034_cxt_k300_otp_info.wb_flag|0x02;
			break;
		default :
			break;
		}
		...
		{
		case 0x00:
			LOG_INF("GC8034_CXT_K300__OTP_GOLDEN group %d is Empty !!\n", index + 1);
			gc8034_cxt_k300_otp_info.golden_flag = gc8034_cxt_k300_otp_info.golden_flag|0x00;					
			break;
		case 0x40:
			LOG_INF("GC8034_CXT_K300__OTP_GOLDEN group %d is Valid !!\n", index + 1);						
			check = 0;
			gc8034_cxt_k300_read_otp_group(9, GOLDEN_ROM_START + index * GOLDEN_WIDTH, &golden[0], GOLDEN_WIDTH);
			...
			{
				gc8034_cxt_k300_otp_info.golden_rg = (golden[0] | ((golden[1] & 0xf0) << 4)) > 0 ?
					(golden[0] | ((golden[1] & 0xf0) << 4)) : RG_TYPICAL;
				gc8034_cxt_k300_otp_info.golden_bg = (((golden[1] & 0x0f) << 8) | golden[2]) > 0 ?
					(((golden[1] & 0x0f) << 8) | golden[2]) : BG_TYPICAL;
				gc8034_cxt_k300_otp_info.golden_flag = gc8034_cxt_k300_otp_info.golden_flag|0x01;	
			}
			else
			{
				LOG_INF("GC8034_CXT_K300__OTP_GOLDEN Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x80:
		case 0xc0:	
			LOG_INF("GC8034_CXT_K300__OTP_GOLDEN group %d is Invalid !!\n", index + 1);	
			gc8034_cxt_k300_otp_info.golden_flag = gc8034_cxt_k300_otp_info.golden_flag|0x02;			
			break;
		default :
			break;
		}	
		...
		{
			case 0x00:
				LOG_INF("GC8034_CXT_K300__OTP_AF group %d is Empty !!\n", index + 1);
				gc8034_cxt_k300_otp_info.af_flag = gc8034_cxt_k300_otp_info.af_flag|0x00;					
				break;
			case 0x04:
				printk("GC8034_CXT_K300__OTP_AF group %d is Valid !!\n", index + 1);						
				check = 0;
			gc8034_cxt_k300_read_otp_group(3, AF_ROM_START + index * AF_WIDTH, &af[0], AF_WIDTH);
				...
				{
				gc8034_cxt_k300_otp_info.af_infinity = ((af[0] << 4) & 0x0f00) + af[1];
				gc8034_cxt_k300_otp_info.af_macro = ((af[0] << 8) & 0x0f00) + af[2];
					gc8034_cxt_k300_otp_info.af_flag = gc8034_cxt_k300_otp_info.af_flag|0x01;	
	            otp_gc8034_cxt_k300_af_data[0] = gc8034_cxt_k300_otp_info.af_infinity;
				otp_gc8034_cxt_k300_af_data[1] = gc8034_cxt_k300_otp_info.af_macro;}
				else
				{
					LOG_INF("GC8034_CXT_K300__OTP_AF Check sum %d Error !!\n", index + 1);
				}
				break;
			case 0x08:
			case 0x0c:	
				LOG_INF("GC8034_CXT_K300__OTP_AF group %d is Invalid !!\n", index + 1);	
				gc8034_cxt_k300_otp_info.af_flag = gc8034_cxt_k300_otp_info.af_flag|0x02;			
				break;
			default :
				break;
		}		
	...
	/* LSC */
	flag_lsc = gc8034_cxt_k300_read_otp(3, 0x43);
	gc8034_cxt_k300_otp_info.lsc_flag = 2;
	for(index=0;index<2;index++)
	{
      ...
      switch((flag_lsc<<(2 * index))&0x0c)
		{
		case 0x00:
			 LOG_INF("GC8034_CXT_K300__OTP_LSC group %d is Empty !\n",index + 1); 
			 break;
		case 0x04:
			LOG_INF("GC8034_OTP_LSC group %d is Valid !\n", index + 1);
			gc8034_cxt_k300_otp_info.lsc_flag = index;
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("GC8034_CXT_K300__OTP_LSC group %d is Invalid !!\n", index + 1);			
			break;
		default :
			break;
      	}
	...
	LOG_INF("GC8034_CXT_K300__OTP_INFO:module_id=0x%x\n",gc8034_cxt_k300_otp_info.module_id);
	LOG_INF("GC8034_CXT_K300__OTP_INFO:lens_id=0x%x\n",gc8034_cxt_k300_otp_info.lens_id);
	LOG_INF("GC8034_CXT_K300__OTP_INFO:vcm_id=0x%x\n",gc8034_cxt_k300_otp_info.vcm_id);
	LOG_INF("GC8034_CXT_K300__OTP_INFO:vcm_driver_id=0x%x\n",gc8034_cxt_k300_otp_info.vcm_driver_id);
	LOG_INF("GC8034_CXT_K300__OTP_INFO:data=%d-%d-%d\n",gc8034_cxt_k300_otp_info.year,gc8034_cxt_k300_otp_info.month,gc8034_cxt_k300_otp_info.day);
	LOG_INF("GC8034_CXT_K300__OTP_WB:r/g=0x%x\n",gc8034_cxt_k300_otp_info.rg_gain);
	LOG_INF("GC8034_CXT_K300__OTP_WB:b/g=0x%x\n",gc8034_cxt_k300_otp_info.bg_gain);
	LOG_INF("GC8034_CXT_K300__OTP_GOLDEN:golden_rg=0x%x\n",gc8034_cxt_k300_otp_info.golden_rg);
	LOG_INF("GC8034_CXT_K300__OTP_GOLDEN:golden_bg=0x%x\n",gc8034_cxt_k300_otp_info.golden_bg);	
	LOG_INF("GC8034_CXT_K300__OTP_AF:infitiy=0x%x\n", gc8034_cxt_k300_otp_info.af_infinity);
	LOG_INF("GC8034_CXT_K300__OTP_AF:macro=0x%x\n", gc8034_cxt_k300_otp_info.af_macro);
#endif

	/*chip regs*/
	gc8034_cxt_k300_otp_info.reg_flag = gc8034_cxt_k300_read_otp(2, 0x4e);

	if(gc8034_cxt_k300_otp_info.reg_flag==1)
		for(i=0;i<5;i++)
		{
			temp = gc8034_cxt_k300_read_otp(2, 0x4f + 5 * i);
		for(j=0;j<2;j++)
			if(((temp>>(4*j+3))&0x01)==0x01)
				{
				gc8034_cxt_k300_otp_info.reg_page[gc8034_cxt_k300_otp_info.reg_num] = (temp>>(4*j))&0x03;
				gc8034_cxt_k300_otp_info.reg_addr[gc8034_cxt_k300_otp_info.reg_num] =
						gc8034_cxt_k300_read_otp(2, 0x50 + 5 * i + 2 * j);
					gc8034_cxt_k300_otp_info.reg_value[gc8034_cxt_k300_otp_info.reg_num] =
						gc8034_cxt_k300_read_otp(2, 0x50 + 5 * i + 2 * j + 1);
					gc8034_cxt_k300_otp_info.reg_num++;
				}
			}
#ifdef GC8034_CXT_K300_OTP_DEBUG
	for(i = 0; i < 9; i++) {
		gc8034_cxt_k300_read_otp_group(i, 0, &debug[0], 128);
		for(j = 0; j < 128; j++) {
...
static void gc8034_cxt_k300_gcore_check_prsel(void)
{
	kal_uint8 product_level = 0;

	product_level = gc8034_cxt_k300_read_otp(2, 0x68) & 0x07;
...
static void gc8034_cxt_k300_gcore_update_dd(void)
{
	kal_uint8 state = 0;

	/*TODO*/
	if(0x01 ==gc8034_cxt_k300_otp_info.dd_flag)
	{ 
	   LOG_INF("GC8034_CXT_K300__OTP_AUTO_DD start !\n");
			 write_cmos_sensor(0xfe,0x00);
	         write_cmos_sensor(0x79,0x2e);//enable
	         write_cmos_sensor(0x7b,gc8034_cxt_k300_otp_info.dd_cnt);
...
#ifdef GC8034_CXT_K300_OTP_FOR_CUSTOMER
static void gc8034_cxt_k300_gcore_update_wb(void)
{
	...
	if(0x01==(gc8034_cxt_k300_otp_info.golden_flag&0x01))
	{
		rg_typical=gc8034_cxt_k300_otp_info.golden_rg;
		bg_typical=gc8034_cxt_k300_otp_info.golden_bg;
		LOG_INF("GC8034_CXT_K300__OTP_UPDATE_AWB:rg_typical = 0x%x , bg_typical = 0x%x\n",rg_typical,bg_typical);		
	}
	else
	{
	...
	if(0x01==(gc8034_cxt_k300_otp_info.wb_flag&0x01))
	{	
		r_gain_current = 2048 * rg_typical/gc8034_cxt_k300_otp_info.rg_gain;
		b_gain_current = 2048 * bg_typical/gc8034_cxt_k300_otp_info.bg_gain;
		g_gain_current = 2048;
		...
		LOG_INF("GC8034_CXT_K300__OTP_UPDATE_AWB:r_gain_current = 0x%x , b_gain_current = 0x%x , base_gain = 0x%x \n",r_gain_current,b_gain_current,base_gain);

		r_gain = 0x400 * r_gain_current / base_gain;
		g_gain = 0x400 * g_gain_current / base_gain;
		b_gain = 0x400 * b_gain_current / base_gain;
		LOG_INF("GC8034_CXT_K300__OTP_UPDATE_AWB:r_gain = 0x%x , g_gain = 0x%x , b_gain = 0x%x \n",r_gain,g_gain,b_gain);
...
static void gc8034_cxt_k300_gcore_update_lsc(void)
{	
	kal_uint8 state = 0;

	if (0x02 != gc8034_cxt_k300_otp_info.lsc_flag) {
			 write_cmos_sensor(0xfe,0x00);
	         write_cmos_sensor(0x78,0x9b);//row and col
	         write_cmos_sensor(0x79,0x0d);//enable
	  		 write_cmos_sensor(0x7a,LSC_NUM);//lsc_num
		write_cmos_sensor(0x7c, LSC_ADDR[gc8034_cxt_k300_otp_info.lsc_flag * 2]);
		write_cmos_sensor(0x7d, LSC_ADDR[gc8034_cxt_k300_otp_info.lsc_flag * 2 + 1]);
			 ...
			 write_cmos_sensor(0x79,0x00);
			 write_cmos_sensor(0xfe,0x01);
	  		 write_cmos_sensor(0xa0,0x13);//[0]lsc en//11 2017.5.26
	  		 write_cmos_sensor(0xfe,0x00);
	}
}
#endif

static void gc8034_cxt_k300_gcore_update_chipversion(void)
{
	kal_uint8 i = 0;

	LOG_INF("GC8034_CXT_K300__OTP_UPDATE_CHIPVERSION:reg_num = %d\n",gc8034_cxt_k300_otp_info.reg_num);

	if(gc8034_cxt_k300_otp_info.reg_flag)
	{
		for(i=0;i<gc8034_cxt_k300_otp_info.reg_num;i++) 
		{
			write_cmos_sensor(0xfe,gc8034_cxt_k300_otp_info.reg_page[i]);
			write_cmos_sensor(gc8034_cxt_k300_otp_info.reg_addr[i] ,gc8034_cxt_k300_otp_info.reg_value[i]);
			LOG_INF("GC8034_OTP_UPDATE_CHIP_VERSION: P%d:0x%x -> 0x%x\n",
			gc8034_cxt_k300_otp_info.reg_page[i], gc8034_cxt_k300_otp_info.reg_addr[i], gc8034_cxt_k300_otp_info.reg_value[i]);
		}
	}
}

static void gc8034_cxt_k300_gcore_update_otp(void)
{
	gc8034_cxt_k300_gcore_update_dd();
	gc8034_cxt_k300_gcore_check_prsel();
#ifdef GC8034_CXT_K300_OTP_FOR_CUSTOMER	
	gc8034_cxt_k300_gcore_update_wb();
    gc8034_cxt_k300_gcore_update_lsc();
#endif	
	gc8034_cxt_k300_gcore_update_chipversion();	
}

static void gc8034_cxt_k300_gcore_enable_otp(kal_uint8 state)
{
	...
	{ 
		...
		LOG_INF("GC8034_CXT_K300__OTP: Enable OTP!\n");		
	}
	else			
	{
		otp_en = otp_en & 0xf7;
		...
		LOG_INF("GC8034_CXT_K300__OTP: Disable OTP!\n");
	}

}


static void gc8034_cxt_k300_gcore_identify_otp(void)
{	
	gc8034_cxt_k300_gcore_enable_otp(otp_open);
	gc8034_cxt_k300_gcore_read_otp_info();
	gc8034_cxt_k300_gcore_update_otp();
	gc8034_cxt_k300_gcore_enable_otp(otp_close);
}
...
static void set_dummy(void)
{
	...
	LOG_INF("ackman dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void set_max_framerate(kal_uint16 framerate, kal_bool min_framelength_en)
{
	...
	LOG_INF("ackman Enter set_max_framerate = %d \n", frame_length);
...
{	
	...
	realtime_fps = imgsensor.pclk / imgsensor.line_length * 10 / imgsensor.frame_length;
	 LOG_INF("ackman V2 realtime_fps =%d\n", realtime_fps);
    ...
    LOG_INF("ackman 20171125! shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
{	
	...
	kal_uint8 i = 0;
	LOG_INF("ackman Enter set_gain = %d \n", gain);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xd2, 0xc3);
	write_cmos_sensor(0xd8,0x60); //dacin offset
	write_cmos_sensor(0xd9,0xff); //rampr width
	write_cmos_sensor(0xda,0x0e);
	write_cmos_sensor(0xdb,0xb0); //ramps width
	...
	write_cmos_sensor(0xad,0x30); //binning and scalar
	...
	write_cmos_sensor(0x43,0x08); //add_offset 
	write_cmos_sensor(0x4e,0x0f); //row_bits[15:8]
	write_cmos_sensor(0x4f,0xf0); //row_bits[7:0]
	...
	write_cmos_sensor(0x12,0xf8); //lwc 1632*5/4
...
{
	LOG_INF("Enter capture_setting20171125!\n");
	fullsize_setting();
}
...
static kal_uint32 open(void)
{
	...
	gc8034_cxt_k300_gcore_identify_otp();
    ...
    spin_unlock(&imgsensor_drv_lock);
    GC8034_CXT_K300_DuringTestPattern = KAL_FALSE; 
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    switch (scenario_id) {
        ...
        default:  //coding with  preview scenario by default
            ...
            spin_unlock(&imgsensor_drv_lock);
		set_dummy();
            LOG_INF("ackman error scenario_id = %d, we use preview scenario \n", scenario_id);
            break;
    }
    return ERROR_NONE;
}
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        case SENSOR_FEATURE_GET_PERIOD:
            *feature_return_para_16++ = imgsensor.line_length;//3002
            *feature_return_para_16 = imgsensor.frame_length;
            *feature_para_len=4;
            break;
        case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
            *feature_return_para_32 = imgsensor.pclk;//3003
            *feature_para_len=4;
            break;
        case SENSOR_FEATURE_SET_ESHUTTER:
            set_shutter(*feature_data);//3004
            break;
        case SENSOR_FEATURE_SET_NIGHTMODE:
            night_mode((BOOL) *feature_data);
            break;
        case SENSOR_FEATURE_SET_GAIN:
            set_gain((UINT16) *feature_data);//3006
            break;
...
UINT32 GC8034_CXT_K300_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    GC8034_CXT_K300_MIPI_RAW_SensorInit    */
