	...
	if (On) {
		...
		PK_DBG("[PowerOn]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);
			...
			if ((pinSetIdx == 0) && (currSensorName
				&& (0 == strcmp(SENSOR_DRVNAME_GC8034_CXT_K300_MIPI_RAW, currSensorName)))) {
		...
		else if ((pinSetIdx == 1) && (currSensorName&& (0 == strcmp(SENSOR_DRVNAME_S5K5E2YA_CXT_K300_MIPI_RAW, currSensorName))))
		{
...
if (TRUE != _hwPowerOn(VCAMD_SUB, VOL_1200)) {
	...
	else if((pinSetIdx == 0) && (currSensorName&& (0 == strcmp(SENSOR_DRVNAME_GC8024_CXT_K300_MIPI_RAW, currSensorName))))
		{
		 ...
		 && (0 == strcmp(SENSOR_DRVNAME_GC5025_RIYONG_K300_MIPI_RAW, currSensorName)))||(currSensorName
		 && (0 == strcmp(SENSOR_DRVNAME_HI556_CXT_K300_MIPI_RAW, currSensorName))))) {
	 /* First Power Pin low and Reset Pin Low */
	 if (GPIO_CAMERA_INVALID != pinSet[1-pinSetIdx][IDX_PS_CMPDN])
		 mtkcam_gpio_set(1-pinSetIdx, CAMPDN,
				 pinSet[1-pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
	 ...
	 mdelay(20);
		...
		else if ((pinSetIdx == 0) && (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5675_MIPI_RAW, currSensorName))))
		{
		...
		else if ((pinSetIdx == 1) && (currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV5675_MIPI_RAW_SUB, currSensorName))))
		{
...
//
	...
	if (TRUE != _hwPowerDown(VCAMD_SUB)) {
     ...
     if ((pinSetIdx == 0) && ((currSensorName &&
			...
			MainCamera_AVDD_PowerDown();			/* VCAM_IO */
			if (TRUE != _hwPowerDown(VCAMIO)) {
				PK_DBG
					("[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO),power id = %d\n",
					 VCAMIO);
