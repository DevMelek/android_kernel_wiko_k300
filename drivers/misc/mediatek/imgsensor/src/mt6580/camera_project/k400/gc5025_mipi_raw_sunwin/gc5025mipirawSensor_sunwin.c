...
#include "gc5025mipirawSensor_sunwin.h"
...
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC5025_SUNWIN_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 retry = 2;
    	...
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			LOG_INF("gc5025 sunwin get_imgsensor_id= 0x%x\n", *sensor_id);
			*sensor_id = *sensor_id + GC5025_SUNWIN_SENSOR_ID_OFFSET;
            		if (*sensor_id == imgsensor_info.sensor_id) {
				gc5025_gcore_identify_otp();
				LOG_INF("gc5025_sunwin module_id = 0x%x\n", gc5025_otp_info.module_id);
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
            sensor_id = return_sensor_id() + GC5025_SUNWIN_SENSOR_ID_OFFSET;
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
