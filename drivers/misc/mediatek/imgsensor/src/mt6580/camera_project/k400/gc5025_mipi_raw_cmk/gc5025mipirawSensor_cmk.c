...
#include "gc5025mipirawSensor_cmk.h"
...
#define gc5025_cmk_OTP_FOR_CUSTOMER

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool gc5025_cmk_DuringTestPattern = KAL_FALSE;

static kal_uint32 Dgain_ratio = 1;
static kal_bool DR_State = KAL_TRUE;
static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC5025_CMK_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
       .checksum_value = 0x1c10cbe1,        //checksum value for Camera Auto Test
...
#ifdef gc5025_cmk_OTP_FOR_CUSTOMER
#define RG_TYPICAL    		0x4BF
#define BG_TYPICAL			0x506
...
static void gc5025_cmk_gcore_read_otp_info(void)
{
	...
	{
	case 0x00:
		LOG_INF("0x00 is Empty !!\n");
		gc5025_cmk_otp_info.dd_flag = 0x00;
		break;
	case 0x01:	
		LOG_INF("0x01 is Valid!!\n");
		...
		{
			LOG_INF("DD check sum correct! checksum = 0x%x\n", ddchecksum);
		}
		else
		{
			LOG_INF("DD check sum error! otpchecksum = 0x%x, sum = 0x%x\n", ddchecksum, (check % 255 + 1));
		}

		for (i=0; i<total_number; i++)
		{
			LOG_INF("DD:index = %d, data[0] = %x , data[1] = %x , data[2] = %x ,data[3] = %x \n",\
			...
			{
				LOG_INF("DD:check_id[%d] = %x,checkid error!!\n", i, ddtempbuff[4 * i + 3] & 0xf0);
			}
		}
...
/*For Chip Version*/
	LOG_INF("CHIPVESION: flag_chipversion = 0x%x\n",flag_chipversion);

	switch ((flag_chipversion>>4) & 0x03)
	{
	case 0x00:
		LOG_INF("0x00 CHIPVERSION is Empty !!\n");
		break;
	case 0x01:
		LOG_INF("0x01 CHIPVERSION is Valid !!\n");
		gc5025_cmk_select_page_otp(otp_page1);		
		i = 0;
		do {
...
//INFO&WB
	for (index=0; index<2; index++)
	{
		...
		{
		case 0x00:
			LOG_INF("0x00 group %d is Empty !!\n", index + 1);
			break;
		case 0x01:
			LOG_INF("0x01 group %d is Valid !!\n", index + 1);
			check = 0;
		...
		{
		case 0x00:
			LOG_INF("0x00 WB group %d is Empty !!\n", index + 1);
			gc5025_cmk_otp_info.wb_flag = gc5025_cmk_otp_info.wb_flag|0x00;
			break;
		case 0x01:
			LOG_INF("0x01 WB group %d is Valid !!\n", index + 1);	
			check = 0;
		...
		case 0x02:
		case 0x03:	
			LOG_INF("0x02&0x03 WB group %d is Invalid !!\n", index + 1);			
		...
		{
		case 0x00:
			LOG_INF("0x00_GOLDEN group %d is Empty !!\n", index + 1);
			gc5025_cmk_otp_info.golden_flag = gc5025_cmk_otp_info.golden_flag|0x00;					
			break;
		case 0x01:
			LOG_INF("0x01_GOLDEN group %d is Valid !!\n", index + 1);						
			check = 0;
			...
			{
				LOG_INF("0x01_GOLDEN Check sum %d Error !!\n", index + 1);
			}
			break;
		case 0x02:
		case 0x03:	
			LOG_INF("0x02&0x03_GOLDEN group %d is Invalid !!\n", index + 1);	
...
static void gc5025_cmk_gcore_update_dd(void)
{
	...
	for (i=0; i<gc5025_cmk_otp_info.dd_cnt; i++)
		...
		for (i=0; i<gc5025_cmk_otp_info.dd_cnt; i++)
		{
			...
			LOG_INF("GC val0 = 0x%x , val1 = 0x%x , val2 = 0x%x \n",temp_val0,temp_val1,temp_val2);
...
static int gc5025_cmk_gcore_check_version(void)
{
    ...
    flag_GC5025A = gc5025_cmk_read_otp(0x27);
...
{	
	...
	if ((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))	
	{
		...
		LOG_INF("analogic gain 1x, gc5025_cmk_subMIPI add pregain = %d\n",temp);
	}
	else 
	{
		write_cmos_sensor(0xfe,  0x00);
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("analogic gain 1.4x, gc5025_cmk_subMIPI add pregain = %d\n",temp);		
	}
	return gain;
}    /*    set_gain  */
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 retry = 2;
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			LOG_INF("gc5025 cmk get_imgsensor_id= 0x%x\n", *sensor_id);
			*sensor_id = *sensor_id + GC5025_CMK_SENSOR_ID_OFFSET;
            		if (*sensor_id == imgsensor_info.sensor_id) {
				gc5025_cmk_gcore_identify_otp();
				LOG_INF("gc5025_cmk module_id = 0x%x\n", gc5025_cmk_otp_info.module_id);
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
            sensor_id = return_sensor_id() + GC5025_CMK_SENSOR_ID_OFFSET;
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
    ...
    spin_unlock(&imgsensor_drv_lock);
    gc5025_cmk_DuringTestPattern = KAL_FALSE; 
