...
#include "ov8856mipirawSensor_sunwin.h"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8856_SUNWIN_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	.checksum_value = 0x7f73eeb0,		//checksum value for Camera Auto Test
	.pre = {
...
static void sensor_init(void)
{
    ...
    */
	...
	write_cmos_sensor(0x366d, 0x00);
		...
		write_cmos_sensor(0x502e, 0x00);//HV_Mirror
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x380f, 0xb2); //0xde);
		...
		write_cmos_sensor(0x3813, 0x02);
		write_cmos_sensor(0x3814, 0x03);
		write_cmos_sensor(0x3820, 0xd6);//HV_Mirror
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150) {
		...
		write_cmos_sensor(0x3820, 0xC6);//HV_Mirror
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			LOG_INF("ov8856 sunwin get_imgsensor_id= 0x%x\n", *sensor_id);
			*sensor_id = *sensor_id + OV8856_SUNWIN_SENSOR_ID_OFFSET;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + OV8856_SUNWIN_SENSOR_ID_OFFSET;
			if (sensor_id == imgsensor_info.sensor_id) {				
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);	  
				break;
			}	
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	sensor_info->SensorHorFOV = 70;
	sensor_info->SensorVerFOV = 57;
