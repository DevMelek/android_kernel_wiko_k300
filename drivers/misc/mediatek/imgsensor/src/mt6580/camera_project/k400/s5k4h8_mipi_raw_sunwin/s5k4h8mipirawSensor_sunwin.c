...
extern void otp_update_wb(void);
extern int S5K4H8_read_OTP(void);
#endif
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = S5K4H8_SUNWIN_SENSOR_ID,
    .checksum_value = 0xeaa01494,
    .pre = {
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor(0x0340, 0x09BA);
    ...
    write_cmos_sensor(0x030E, 0x00A8);
...
static void normal_capture_setting(void)
{
    ...
    write_cmos_sensor(0x030E, 0x00A8);
    write_cmos_sensor(0x3008, 0x0000);
    write_cmos_sensor(0x34E4, 0x0078);
    write_cmos_sensor(0x0100, 0x0100);
    check_output_stream_on();
#ifdef S5K4H8_OTP_USE
    otp_update_wb();
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            *sensor_id = read_cmos_sensor(0x6F12);
            LOG_INF("s5k4h8 sunwin get_imgsensor_id= 0x%x\n", *sensor_id);
            *sensor_id = *sensor_id + S5K4H8_SUNWIN_SENSOR_ID_OFFSET;
            if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                check_output_stream_on();
                //mdelay(10);
                if (S5K4H8_read_OTP()) {//Module Vendor ID
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            write_cmos_sensor(0x602C,0x4000);
            write_cmos_sensor(0x602E,0x0000);
            sensor_id = read_cmos_sensor(0x6F12) + S5K4H8_SUNWIN_SENSOR_ID_OFFSET;
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
