...
static char *mstar_get_vendor_name(int vendor_id);
...
static int /*__devinit*/ tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_err(" mstar_drv_mtk.c TPD probe\n");
    
    TPD_DMESG("TPD probe\n");   
    
    if ( tpd_load_status )
    {
        pr_err(" mstar_drv_mtk.c Already probed a TP, needn't to probe any more!");
        return -1;
    }
    ...
    {
        pr_err(" mstar_drv_mtk.c  %s(%d), Probe FAIL \n", __func__, __LINE__);
	return -1;
    }	
    ...
    CTP_DBG("msg2238a_vendor_name = %s \n", msg2238a_vendor_name);

    tpd_fw_version = nMinor;
    sprintf(tpd_desc, "%s", mstar_get_vendor_name(nMajor));
    ...
    pr_err(" mstar_drv_mtk.c  %s(%d), TPD probe done \n", __func__, __LINE__);
...
static char *mstar_get_vendor_name(int vendor_id)
{
    ...
    switch(vendor_id)
    {
        case MSTAR_CTP_VENDOR_HOLITECH:
	  return "HOLITECH";
            break;
...
void mstar_tpd_get_fw_version( char * fw_vendor_numb )
{
    sprintf(fw_vendor_numb, "%x", tpd_fw_version);
}

void mstar_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
...
static struct tpd_driver_t tpd_device_driver = {
    ...
    .tpd_get_fw_version = mstar_tpd_get_fw_version,
    .tpd_get_fw_vendor_name = mstar_tpd_get_fw_vendor_name,
    //.tpd_ftm_force_update=mstar_ftm_force_update,
};
