...
static  void tpd_down(tinno_ts_data *ts, int x, int y, int pressure, int trackID)
{
    ...
    //BEGIN<><20161012><tp report button key>wangyanhui | v3720
    #ifdef TPD_HAVE_BUTTON
    ...
    #endif
    //END<><20161012><tp report button key>wangyanhui | v3720
...
static  int tpd_up(tinno_ts_data *ts, int x, int y, int pressure, int trackID)
{
    ...
    //if (FACTORY_BOOT == get_boot_mode() || RECOVERY_BOOT == get_boot_mode())//LINE<><20161012><tp report button key | v3720
    {
        tpd_button(x, y, 0);
    }
    return 0;
...
#ifdef CONFIG_TGESTURE_FUNCTION //v3720
...
#if defined(tp_update_v3710) //v3720
...
#elif CONFIG_TOUCHSCREEN_POWER_DOWN_WHEN_SLEEP
        fts_6x06_hw_init();
