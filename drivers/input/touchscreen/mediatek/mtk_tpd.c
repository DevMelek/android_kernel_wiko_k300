...
extern void tfrf_write_reg(char data);
int tprf_value = 0;
extern int ret_rf_data;
#endif
static ssize_t tprf_status_info_show(struct device_driver *ddri, char *buf)
{
	TPD_DMESG("[tprf_dev] tprf_status=%d\n");
    #ifdef CONFIG_PROJECT_K300
...
static ssize_t tprf_status_info_store(struct device_driver * ddri,char * buf,size_t count)
{
       #ifdef CONFIG_PROJECT_K300
       tprf_value=*buf -48;
       TPD_DMESG("[tprf_dev] %s *buf = %c :\n ", __func__,*buf);
       tfrf_write_reg(tprf_value);
       #endif
	return count;
}
static DRIVER_ATTR(tprf_status_info, 0664, tprf_status_info_show,  tprf_status_info_store);
...
//tinno add end
#ifdef CONFIG_PROJECT_K300
atomic_t tprf_write_flag=ATOMIC_INIT(1);
#define TPRF_CONFIG_PROC_FILE     "tprf_config"
#define TPRF_CONFIG_MAX_LENGTH       240
static char config[TPRF_CONFIG_MAX_LENGTH] = {0};


static ssize_t tprf_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char *ptr = page;
	...
	TPD_DMESG("[tprf_dev] tprf_status=%d\n");
      ...
      ptr += sprintf(ptr, "0x%d ", ret_rf_data);
      printk("tprf_config_read_proc:%s\n",ptr);
       *ppos += ptr - page;
     return (ptr - page) ;
}
static ssize_t tprf_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    s32 ret = 0;

    TPD_DMESG("====LGC=========TGesture_config_write_procwrite count %d\n", count);

    if (count > TPRF_CONFIG_MAX_LENGTH  )
    {
        TPD_DMESG("size not match [%d:%d]\n", TPRF_CONFIG_MAX_LENGTH, count);
        return -EFAULT;
    }
...
if(atomic_read(&tprf_write_flag))
{
     atomic_set(&tprf_write_flag,0);
     tprf_value=config[0]-48;
     TPD_DMESG("===== TGesture_config_write_proc%d=====",tprf_value);
     atomic_set(&tprf_write_flag,1);
}
    tfrf_write_reg(tprf_value);
    return count;
}
static const struct file_operations tprf_proc_ops = {
    .owner = THIS_MODULE,
    .read = tprf_config_read_proc,
    .write = tprf_config_write_proc,
};
static struct proc_dir_entry *tprf_config_proc = NULL;
#endif  // #ifdef CONFIG_PROJECT_K300
...
//tinno add end
 ...
 // Create proc file system
       tprf_config_proc = proc_create(TPRF_CONFIG_PROC_FILE, 0666, NULL, &tprf_proc_ops);
     if (tprf_config_proc == NULL)
    {
        TPD_DMESG("create_proc_entry %s failed\n", TPRF_CONFIG_PROC_FILE);
     }
    else
    {
        TPD_DMESG("create proc entry %s success", TPRF_CONFIG_PROC_FILE);
    }
#endif
	return 0;
}
