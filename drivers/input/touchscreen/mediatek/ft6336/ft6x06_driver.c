    ...
    switch(vendor_id)
    {
	 ...
	 break;
        ...
        case FTS_CTP_VENDOR_YIJIAN:
            ...
            break;  
#endif
        case FTS_CTP_VENDOR_YIJIAN_PAIZU:
            return "YIJIAN_P";
            break;  
        case FTS_CTP_VENDOR_ZHONGGUAGNDIAN:
            return "ZHONGGUAGNDIAN";
            break;  
		...
		if (NORMAL_BOOT == get_boot_mode()){
                    ...
                    else if(panel_vendor_buff == FTS_CTP_VENDOR_YIJIAN)
			{
				ft6x06_tp_upgrade(ftbin_yj, sizeof(ftbin_yj));
                    ...
                    else if(panel_vendor_buff == FTS_CTP_VENDOR_YIJIAN_PAIZU)
			{
				ft6x06_tp_upgrade(ftbin_yj_pz, sizeof(ftbin_yj_pz));
			}
                    else if(panel_vendor_buff == FTS_CTP_VENDOR_ZHONGGUAGNDIAN)
			{
				ft6x06_tp_upgrade(ftbin_zgd, sizeof(ftbin_zgd));
			}
		}
		else{
 		  CTP_DBG("update_firmware_thread : Dont upgrade");   
