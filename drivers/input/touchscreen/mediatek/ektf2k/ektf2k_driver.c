...
//LINE<JIRA_ID><DATE20180301><add tp info>zenghaihui
...
#define ELAN_GESTURE_HEAD 0x88
enum
{
    ELAN_GESTURE_O  =0x01,
    ELAN_GESTURE_M  =0x09,
    ELAN_GESTURE_C  =0x05,
    ELAN_GESTURE_DOUBLE_CLICK = 0x0A,
};
#endif
...
#define MTK_TP_DEBUG(fmt, args ...)  pr_err("Elan_tp %s (line:%d) :" fmt "\r\n", __func__, __LINE__, ##args)
#else
...
static dma_addr_t gpDMAFWBuf_pa;
...
static int elan_i2c_dma_send_data(struct i2c_client *client, uint8_t *buf,uint8_t len)
{
    ...
    addr = client->addr ;
    client->addr |= I2C_DMA_FLAG;
    uint8_t *pWriteData = (uint8_t)gpDMABuf_va;
    if(!pWriteData)
    {
        MTK_TP_DEBUG("[elan] dma_alloc_coherent failed!\n");
        return -1;
    }
    if(copy_from_user(pWriteData, ((void*)buf), len))
    {
    	MTK_TP_DEBUG("[elan] copy_from_user failed!\n");
      return -1;
    }
...
static int elan_i2c_dma_fw_recv_data(struct i2c_client *client, uint8_t *buf,uint8_t len)
{
    ...
    rc = i2c_master_recv(client, (unsigned char *)gpDMAFWBuf_pa, len);
    MTK_TP_DEBUG("[elan] elan_i2c_dma_recv_data rc=%d!\n",rc);
    if(copy_to_user(buf, pReadData, len))
    {
    		MTK_TP_DEBUG("[elan] copy_to_user failed!\n");
        return -1;
    }
    client->addr = addr;
    return rc;
...
static int elan_i2c_dma_fw_send_data(struct i2c_client *client, uint8_t *buf,uint8_t len)
{
    int rc;
    unsigned short addr = 0;
    uint8_t *pWriteData = 0;
    addr = client->addr ;
    client->addr |= I2C_DMA_FLAG;
    pWriteData = gpDMAFWBuf_va;
    if(!pWriteData)
    {
        MTK_TP_DEBUG("[elan] dma_alloc_coherent failed!\n");
        return -1;
...
if (copy_from_user(pWriteData, ((void*)buf), len))
    ...
    rc = i2c_master_send(client, (unsigned char *)gpDMAFWBuf_pa, len);
    MTK_TP_DEBUG("[elan] elan_i2c_dma_send_data rc=%d!\n",rc);
    client->addr = addr;
    return rc;
}
...
//add by zcm
#ifdef GESTURE_MODE
static void elan_exit_gesture_mode(struct i2c_client *client)
{
    
    uint8_t cmd[] = {CMD_W_PKT, 0x58, 0x00, 0x01};
    int size = sizeof(cmd);
    if(i2c_master_send(client, cmd, size) != sizeof(cmd))
    {
    	 MTK_TP_DEBUG("[ELAN] ERROR: elan_exit_gesture_mode fail!\n");
    }else{
    	 MTK_TP_DEBUG("[ELAN] SUCCESS: elan_exit_gesture_mode OK!\n");
  } 
}

extern u8 gTGesture;
static void elan_gesture_report(struct elan_ktf2k_ts_data *ts, uint8_t *buf)
{

MTK_TP_DEBUG("[ELAN] geroge buf[1] = %x\n",buf[1]);

    switch (buf[1]) {
    	case ELAN_GESTURE_O:
    	            //customer do
    	            gTGesture = 'o';
                    input_report_key(ekt_key_dev,251, 1);
		    input_sync(ekt_key_dev);
                    input_report_key(ekt_key_dev,251, 0);
                    input_sync(ekt_key_dev);
    	            break;
    	case ELAN_GESTURE_C:
    	            //customer do
    	...
    	case ELAN_GESTURE_DOUBLE_CLICK:
		    ...
		    input_sync(ekt_key_dev);
    	            ...
    	            break;
    	default:
                MTK_TP_DEBUG("[elan] %s: unknow packet %x\n", __func__, buf[1]);
                return;
    }
}
#endif
...
static int elan_ktf2k_ts_recv_data(struct i2c_client *client, uint8_t *buf)
{
    ...
    rc = i2c_master_recv(client, (u8 *)(uintptr_t)gpDMABuf_pa, bytes_to_recv);
...
#ifdef GESTURE_MODE
       if(ELAN_GESTURE_HEAD == buf[0]){
            elan_gesture_report(private_ts, buf);
        }
#endif
#ifdef TP_PROXIMITY_SENSOR_NEW
    else if(buf[0] == 0xFA)
...
static int update_fw_handler(void *unused)
{
    ...
    MTK_TP_DEBUG("geroge FW_ID=0x%x,   New_FW_ID=0x%x \n",  FW_ID, New_FW_ID);
    MTK_TP_DEBUG("geroge FW_VERSION=0x%x,   New_FW_VER=0x%x \n",  FW_VERSION  , New_FW_VER);
#endif

    /* for firmware auto-upgrade*/
   if (New_FW_ID    ==  FW_ID )
    {
MTK_TP_DEBUG("geroge upgrade! 11");
       // if (New_FW_VER > (FW_VERSION))
       if ((New_FW_VER) > (FW_VERSION)||(ftm_ektf2k_force_update))
{MTK_TP_DEBUG("geroge upgrade! 22");
...
MTK_TP_DEBUG("geroge  irq---tpd_load_status  = %d\n",tpd_load_status);
...
static void get_vendor_info(void)
{
    ...
    for(i=0; i < vendor_num; i++)
    {
        ...
        {
            //LINE<JIRA_ID><DATE20180209><disable tp upgrade temp>zenghaihui
...
static int tpd_irq_ent_gpio_parse(void)
{
	...
	if (node) {
        ...
        MTK_TP_DEBUG("touch_irq = %d, tpd_eint_gpio = %d", touch_irq, tpd_eint_gpio);
...
static int tpd_irq_registration(void)
{
        ...
        if(touch_irq !=0)
        {
            ...
            if (ret > 0)
                MTK_TP_DEBUG("tpd request_irq IRQ LINE NOT AVAILABLE!.");
        }
        else
        {
            MTK_TP_DEBUG("tpd eint number 0 !!! ");
        }
#else
...
//LINE<JIRA_ID><DATE20180301><add tp info>zenghaihui
#ifdef CONFIG_TINNO_PRODUCT_INFO
int ektf2k_get_tp_fw_info(char *buf, void *arg0) 
{
    return sprintf(buf, 
        "%s-%s-%s-%x",
        tpd_desc,
...
pr_err(" geroge ekt tpd_load_status = %d\n",tpd_load_status);
  if ( tpd_load_status )
    {
      pr_err(" ektf2k_driver.c Already probed a TP, needn't to probe any more!");
        return -1;
    }
...
read_retry:
 ...
 //   }
  ...
  ekt_key_dev= input_allocate_device();
if (! ekt_key_dev) 
{
	MTK_TP_DEBUG("[fts]SY_key_dev: fail!\n");
}
   __set_bit(EV_KEY,  ekt_key_dev->evbit);
   __set_bit(251,	ekt_key_dev->keybit);
__set_bit(KEY_POWER,  ekt_key_dev->keybit);
ekt_key_dev->id.bustype = BUS_HOST;
ekt_key_dev->name = "EKT_GESTURE";
if(input_register_device(ekt_key_dev))
{
	MTK_TP_DEBUG("[ekt]ekt_key_dev register : fail!\n");
}else
{
	MTK_TP_DEBUG("[ekt]ekt_key_dev register : success!!\n");
} 
#endif
...
#ifdef CONFIG_TINNO_PRODUCT_INFO
        {
            FULL_PRODUCT_DEVICE_CB(ID_TP, ektf2k_get_tp_fw_info, NULL);
        }
#endif


	tpd_load_status = 1;
pr_err("geroge  prob ---tpd_load_status  = %d\n",tpd_load_status);
...
static void elan_touch_esd_func(struct work_struct *work)
{
	...
	{
		goto out_esd;
		MTK_TP_DEBUG("now is in suspend ,not check esd!\n");
	}
...
void ektf2k_ftm_force_update(char * ftm_update){
   ...
   // New_FW_VER = file_fw_data[31697]<<8  | file_fw_data[31696] ;
    ...
    New_FW_VER = file_fw_data[0xBDD1]<<8  | file_fw_data[0xBDD0] ;
    ...
    if (New_FW_ID   ==  FW_ID)
    {
MTK_TP_DEBUG("geroge upgrade! 1");
        if ((New_FW_VER > FW_VERSION) || (ftm_ektf2k_force_update == 1))
{           MTK_TP_DEBUG("geroge upgrade!");
            Update_FW_One(private_ts->client, RECOVERY);
...
#ifdef GESTURE_MODE
static int elan_enter_gesture_mode(struct i2c_client *client)
{
    int rc = 0;
    
    uint8_t gesture_cmd[] = {CMD_W_PKT, 0x54, 0x10, 0x00};//enter gesture mode cmd
    uint8_t check_gesture[] = {0x53, 0x50, 0x00, 0x01};
    uint8_t recvpwr[4] = {0};
    //1��send cmd into getsture mode
    rc = i2c_master_send(client, gesture_cmd, sizeof(gesture_cmd));

 if (rc != sizeof(gesture_cmd)) {
			MTK_TP_DEBUG("mtk-tpd:[elan]: gesture_cmd[1] = %d enter getsture mode fail\n",gesture_cmd[1]);
    } else {
			MTK_TP_DEBUG("mtk-tpd:[elan]: gesture_cmd[1] = %d enter getsture mode success\n", gesture_cmd[1]);
	  }
	
    mdelay(20);    
    //2��check into gesture or not
    if(i2c_master_send(client, check_gesture, sizeof(check_gesture))!= sizeof(check_gesture)) 
          return -EINVAL;
        
    rc = i2c_master_recv(client, recvpwr, sizeof(recvpwr));
    if (rc != sizeof(recvpwr)) {
			pr_err("[elan] %s: i2c_master_recv recvpwr failed\n", __func__);
    } else {
			pr_err("[elan] %s: recv %x %x %x %x\n", __func__, recvpwr[0],
...
static void tpd_suspend(struct device *h)
{
    ...
    cancel_delayed_work_sync(&esd_work);
			...
			if(bEnTGesture)
			{
				//disable_irq(touch_irq);
				mdelay(50);
				if(elan_enter_gesture_mode(private_ts->client))
				{
					MTK_TP_DEBUG("mtk-tpd:[elan] %s: enter getsture mode fail\n", __func__);
				}
...
//    mdelay(10);
  ...
  //  mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
#ifdef GESTURE_MODE	  
	elan_exit_gesture_mode(private_ts->client);
...
pr_err("geroge  ---tpd_load_status  = %d\n",tpd_load_status);
