...
#else
/** guomingyi add for simulator-ps */
#ifdef CONFIG_SIMULATOR_PS_SUPPORT
            SIMULATOR_PS_REPORT_DATA(tpd_proximity_detect); 
#else
            err = ps_report_interrupt_data(tpd_proximity_detect);
#endif
#endif	
        }
#endif
...
#if defined TPD_PROXIMITY
  printk("TPD_PROXIMITY val_for_tp_ps = %d\n", val_for_tp_ps);
  if (1 == val_for_tp_ps){
#if FTS_MTK_LTE
    ...
    if((err = hwmsen_attach(ID_PROXIMITY, &obj_ps)))
    {
        ...
        CTP_DBG("proxi_fts attach fail = %d\n", err);
    }
    else
    {
        //APS_ERR("proxi_fts attach ok = %d\n", err);
        CTP_DBG("proxi_fts attach ok = %d\n", err);
    }
#else
...
#endif
  }else{  //// val_for_tp_ps = 0
      CTP_DBG("val_for_tp_ps nothing to do\n");
  }
#endif
#endif
...
static int ft6x06_local_init(void)
{
...
static struct alsps_init_info ft6x06_init_info = {
	.name = DEVICE_NAME,
	.init = ft6x06_local_init,
	.uninit = ft6x06_remove,
...
static int __init tpd_driver_init(void)
{
	...
	tpd_driver_add(&tpd_device_driver);
 	if (1 == val_for_tp_ps){	
	alsps_driver_add(&ft6x06_init_info);
 	}
	return 0;
}
