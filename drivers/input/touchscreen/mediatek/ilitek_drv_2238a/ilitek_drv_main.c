...
extern char *ilitek_get_vendor_name(int vendor_id);
...
static ssize_t DrvMainProcfsMpTestCustomisedRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    DBG(&g_I2cClient->dev, "*** %s() Driver version = %s ***, %s\n", __func__, DEVICE_DRIVER_RELEASE_VERSION, ini_path);
...
static ssize_t _DrvProcfsChipTypeRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    nLength = sprintf(nUserTempBuffer, "%d", g_ChipType);
    ...
    else if (g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)
    {
        if (nNum > 0) // nCount >= 1024
        {
            for (i = 0; i < nNum; i ++)
            {
                ...
                memcpy(g_IliTekFwDataBuf+(g_FwDataCount), _gDebugBuf, 1024);
            ...
            if (nRemainder > 0) 
            {
                ...
                memcpy(g_IliTekFwDataBuf+(g_FwDataCount), _gDebugBuf, nRemainder);
...
s32 /*__devinit*/ MsDrvInterfaceTouchDeviceProbe(struct i2c_client *pClient, const struct i2c_device_id *pDeviceId)
{
    ...
    if (nRetVal == -ENODEV)
    {
        ...
        pr_err("*** %s(%d)   device init fail, nRetVal = %d \n", __func__, __LINE__, nRetVal);
        return nRetVal;
    }
	...
	if(g_FwVersionFlag)
	{
		pr_err("*** ILITEK %s() Major = %d, Minor = %d.%d, _gFwVersion = %s ***\n", __func__, nMajor, (nMinor & 0xFF), ((nMinor >> 8) & 0xFF), _gFwVersion);
	}
	else
	{
		pr_err("*** ILITEK %s() Major = %d, Minor = %d, _gFwVersion = %s ***\n", __func__, nMajor, nMinor, _gFwVersion);
	}

    tpd_fw_version = nMinor;
    sprintf(tpd_desc, "%s", ilitek_get_vendor_name(nMajor));
    
    pr_err("*** ILITEK %s() tpd_fw_version = %d, tpd_desc = %s ***\n", __func__, tpd_fw_version, tpd_desc);
    ...
    pr_err("*** %s(%d)   ILITEK/MStar touch driver registered nRetVal = %d ***\n", __func__, __LINE__, nRetVal);
    return nRetVal;
}
