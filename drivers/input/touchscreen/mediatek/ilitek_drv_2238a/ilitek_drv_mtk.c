...
//    const char *vcc_i2c_name = "vcc_i2c";
    ...
    pr_err(" ilitek_drv_mtk.c TPD probe\n");
    
    TPD_DMESG("TPD probe\n");   
    
    if ( tpd_load_status )
    {
        pr_err(" ilitek_drv_mtk.c Already probed a TP, needn't to probe any more!");
        return -1;
    }
    ...
    {
        pr_err(" ilitek_drv_mtk.c TPD probe fail, ret = %d \n", ret);
        return -1;
    }
...
//LINE<JIRA_ID><DATE20180301><add tp info>zenghaihui
    ...
    pr_err(" ilitek_drv_mtk.c TPD probe done\n");
...
char *ilitek_get_vendor_name(int vendor_id)
{
	CTP_DBG(" vendor_id = %x \n", vendor_id);
    
    switch(vendor_id)
    {
        case ILITEK_CTP_VENDOR_HOLITECH:
	  return "HOLITECH";
            break;
...
EXPORT_SYMBOL(ilitek_get_vendor_name);


void ilitek_tpd_get_fw_version( char * fw_vendor_numb )
{
    sprintf(fw_vendor_numb, "%x", tpd_fw_version);
}

void ilitek_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
...
static struct tpd_driver_t tpd_device_driver = {
    ...
    .tpd_get_fw_version = ilitek_tpd_get_fw_version,
    .tpd_get_fw_vendor_name = ilitek_tpd_get_fw_vendor_name,
    //.tpd_ftm_force_update=ilitek_ftm_force_update,
};
